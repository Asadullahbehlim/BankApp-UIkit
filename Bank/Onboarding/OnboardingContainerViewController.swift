...
//  Bank
//
//  Created by Asadullah Behlim on 12/12/22.
//
...
class OnboardingContainerViewController: UIViewController {
    ...
    var currentVC: UIViewController {
        didSet {
            guard let index = pages.firstIndex(of: currentVC)
            else {
                return
            }
            nextButton.isHidden = index == pages.count - 1 //hide on last page
...
extension OnboardingContainerViewController {
    ...
    @objc func nextTapped(sender: UIButton) {
        guard let nextVC = getNextViewController(from: currentVC)
        else {
            return
        }
    ...
    @objc func backTapped(sender: UIButton) {
        guard let previousVC = getNextViewController(from: currentVC)
        else {
            return
        }
